...
public record struct ClientOptions()
{
    ...
    Lazy<string> _baseUrl = new(() =>
        Environment.GetEnvironmentVariable("VAT_SENSE_BASE_URL") ?? EnvironmentUrl.Production
    );
    ...
    /// <see cref="IVatSenseClient.WithRawResponse"/>, don't perform validation until deserialization
    /// is triggered.</para>
    /// </summary>
    ...
    Lazy<string> _username = new(() =>
        Environment.GetEnvironmentVariable("VAT_SENSE_USERNAME")
        ?? throw new VatSenseInvalidDataException(
    ...
    Lazy<string> _password = new(() =>
        Environment.GetEnvironmentVariable("VAT_SENSE_PASSWORD")
        ?? throw new VatSenseInvalidDataException(
