...
public abstract record class ParamsBase
{
    ...
    )
    {
        ...
        if (security.BasicAuth && options.Username != null && options.Password != null)
        {
            request.Headers.Add(
                "Authorization",
                string.Format(
                    ...
                    Convert.ToBase64String(
                        Encoding
                            ...
                            .GetBytes(string.Format("{0}:{1}", options.Username, options.Password))
                    )
                )
            );
        }
    ...
    static string GetUserAgent() => $"{typeof(VatSenseClient).Name}/C# {GetPackageVersion()}";
